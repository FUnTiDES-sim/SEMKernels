 ...
 */
template <typename T> PROXY_HOST_DEVICE T invert3x3(T (&J)[3][3]);
...
PROXY_HOST_DEVICE T determinant(const T (&A)[N][N]);

template <typename T> PROXY_HOST_DEVICE T determinant(const T (&A)[3][3]) {
  return A[0][0] * (A[1][1] * A[2][2] - A[1][2] * A[2][1]) -
         A[0][1] * (A[1][0] * A[2][2] - A[1][2] * A[2][0]) +
         A[0][2] * (A[1][0] * A[2][1] - A[1][1] * A[2][0]);
}

// template <int N, typename T> PROXY_HOST_DEVICE T symDeterminant(T (&B)[3]);

// template <typename T> PROXY_HOST_DEVICE T symDeterminant<2>(T (&B)[3]) {
//   return B[0] * B[1] - B[2] * B[2];
// }

// template <int N, typename T> PROXY_HOST_DEVICE T symDeterminant(T (&B)[6]);

// template <typename T> PROXY_HOST_DEVICE T symDeterminant<3>(T (&B)[6]) {
//   return B[0] * B[1] * B[2] + B[5] * B[4] * B[3] * 2 - B[0] * B[3] * B[3] -
//          B[1] * B[4] * B[4] - B[2] * B[5] * B[5];
// }
// Primary template declaration
// template <int N, typename T>
// PROXY_HOST_DEVICE T symDeterminant(T (&B)[N == 2 ? 3 : 6]);

// // Specialization for N=2
// template <typename T> PROXY_HOST_DEVICE T symDeterminant<2, T>(T (&B)[3]) {
//   return B[0] * B[1] - B[2] * B[2];
// }

// // Specialization for N=3
// template <typename T> PROXY_HOST_DEVICE T symDeterminant<3, T>(T (&B)[6]) {
//   return B[0] * B[1] * B[2] + 2 * B[5] * B[4] * B[3] - B[0] * B[3] * B[3] -
//          B
// [1] * B[4] * B[4] - B[2] * B[5] * B[5];
// }
template <typename T> PROXY_HOST_DEVICE T symDeterminant(T (&B)[3]) {
  return B[0] * B[1] - B[2] * B[2];
}

// 3D version: B has 6 components
template <typename T> PROXY_HOST_DEVICE T symDeterminant(T (&B)[6]) {
  return B[0] * B[1] * B[2] + 2 * B[5] * B[4] * B[3] - B[0] * B[3] * B[3] -
         B[1] * B[4] * B[4] - B[2] * B[5] * B[5];
}
/**
...
template <typename T> PROXY_HOST_DEVICE static auto symInvert(T (&J)[6]) {
