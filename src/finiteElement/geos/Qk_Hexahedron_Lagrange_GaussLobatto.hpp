 ...
 * this file.
 */
template <typename GL_BASIS> class Qk_Hexahedron_Lagrange_GaussLobatto final {
public:
   ...
   */
  PROXY_HOST_DEVICE
  static void calcN(real_t const (&coords)[3], real_t (&N)[numNodes]) {
   ...
   * More precisely, the operator is defined as:
   * \f[
   * R_i = \sum_a^{nSupport} \left( \frac{\partial N_a}{\partial X_j} var_{ij}
   * \right), \f] where \f$\frac{\partial N_a}{\partial X_j}\f$ is the basis
   * function gradient, \f$var_{ij}\f$ is the rank-2 symmetric tensor.
   */
  PROXY_HOST_DEVICE
  static void plusGradNajAij(int const q, real_t const (&invJ)[3][3],
                             real_t const (&var)[6], real_t (&R)[numNodes][3]);
   ...
   */
  PROXY_HOST_DEVICE
  static real_t computeMassTerm(int const q, real_t const (&X)[8][3]);
  ...
  PROXY_HOST_DEVICE static void
  computeStiffnessTerm(int const q, real_t const (&X)[8][3], FUNC &&func);

  /**
   * @brief computes the matrix B in the case of quasi-stiffness (e.g. for
   * pseudo-acoustic case), defined as J^{-T}A_xy J^{-1}/det(J), where J is the
   * Jacobian matrix, and A_xy is a zero matrix except on A_xy(1,1) = 1 and
   * A_xy(2,2) = 1.
   * @param qa The 1d quadrature point index in xi0 direction (0,1)
   * @param qb The 1d quadrature point index in xi1 direction (0,1)
   * @param qc The 1d quadrature point index in xi2 direction (0,1)
  ...
  // GEOS_FORCE_INLINE
  PROXY_HOST_DEVICE
  static void computeBxyMatrix(int const qa, int const qb, int const qc,
                               real_t const (&X)[8][3], real_t (&J)[3][3],
                               real_t (&B)[6]);

  /**
   * @brief computes the non-zero contributions of the d.o.f. indexed by q to
   * the partial-stiffness matrix R, i.e., the superposition matrix of first
   * derivatives in x and y of the shape functions. Warning, the matrix B is
   * obtained by computeBxyMatrix instead of usual one.
   * @param q The quadrature point index
   * @param X Array containing the coordinates of the support points.
   * @param func Callback function accepting three parameters: i, j and R_ij
   */
  ...
  // GEOS_FORCE_INLINE
  PROXY_HOST_DEVICE static void
  computeStiffnessxyTerm(int const q, real_t const (&X)[8][3], FUNC &&func);

  /**
   * @brief computes the matrix B in the case of quasi-stiffness (e.g. for
   * pseudo-acoustic case), defined as J^{-T}A_z J^{-1}/det(J), where J is the
   * Jacobian matrix, and A_z is a zero matrix except on A_z(3,3) = 1.
   * @param qa The 1d quadrature point index in xi0 direction (0,1)
   * @param qb The 1d quadrature point index in xi1 direction (0,1)
   * @param qc The 1d quadrature point index in xi2 direction (0,1)
   * @param X Array containing the coordinates of the support points.
  ...
  // GEOS_FORCE_INLINE
  PROXY_HOST_DEVICE
  static void computeBzMatrix(int const qa, int const qb, int const qc,
                              real_t const (&X)[8][3], real_t (&J)[3][3],
                              real_t (&B)[6]);
   ...
   * @brief computes the non-zero contributions of the d.o.f. indexed by q to
   * the partial-stiffness matrix R, i.e., the superposition matrix of first
   * derivatives in z only of the shape functions. Warning, the matrix B is
   * obtained by computeBzMatrix instead of usual one.
   * @param q The quadrature point index
   * @param X Array containing the coordinates of the support points.
   * @param func Callback function accepting three parameters: i, j and R_ij
   */
  ...
  // GEOS_FORCE_INLINE
  PROXY_HOST_DEVICE static void
  computeStiffnesszTerm(int const q, real_t const (&X)[8][3], FUNC &&func);
  ...
  // GEOS_FORCE_INLINE
  PROXY_HOST_DEVICE static void
  computeGradPhiBGradPhi(int const qa, int const qb, int const qc,
                         real_t const (&B)[6], FUNC &&func);

  /**
   * @brief computes the non-zero contributions of the d.o.f. indexd by q to the
   *   x-part of the first order stiffness matrix R, i.e., the matrix composed
   * of the the product of first derivatives of one shape function i and the
   * shape function j itself.
   * @param q The quadrature point index
   * @param X Array containing the coordinates of the support points.
   * @param func Callback function accepting three parameters: i, j and R_ij
   */
  ...
  // GEOS_FORCE_INLINE
  PROXY_HOST_DEVICE static void
  computeFirstOrderStiffnessTermX(int const q, real_t const (&X)[8][3],
                                  FUNC &&func);
  /**
   * @brief computes the non-zero contributions of the d.o.f. indexd by q to the
   *   y-part of the first order stiffness matrix R, i.e., the matrix composed
   * of the the product of first derivatives of one shape function i and the
   * shape function j itself.
   * @param q The quadrature point index
   * @param X Array containing the coordinates of the support points.
  ...
  // GEOS_FORCE_INLINE
  PROXY_HOST_DEVICE static void
  computeFirstOrderStiffnessTermY(int const q, real_t const (&X)[8][3],
                                  FUNC &&func);
  /**
   * @brief computes the non-zero contributions of the d.o.f. indexd by q to the
   *   z-part of the first order stiffness matrix R, i.e., the matrix composed
   * of the the product of first derivatives of one shape function i and the
   * shape function j itself.
   * @param q The quadrature point index
   * @param X Array containing the coordinates of the support points.
  ...
  template <typename FUNC>
  PROXY_HOST_DEVICE static void
  computeFirstOrderStiffnessTermZ(int const q, real_t const (&X)[8][3],
                                  FUNC &&func);
  /**
   * @brief computes the non-zero contributions of the d.o.f. indexd by q to the
   *   stiffness matrix R for the elastic case, i.e., the superposition matrix
   * of first derivatives of the shape functions. This callback returns the two
   * indices indices i and j of matrix R and the value R[i][j] associated to
   * those two indices.
   * @param q The quadrature point index
   * @param X Array containing the coordinates of the support points.
   * @param stiffnessVal Callback function accepting three parameters: i, j and
   * R_ij
   */
  template <typename FUNC>
  PROXY_HOST_DEVICE static void
  computeFirstOrderStiffnessTerm(int const q, real_t const (&X)[8][3],
                                 FUNC &&stiffnessVal);
  ...
  PROXY_HOST_DEVICE
  static void
  applyTransformationToParentGradients(int const q, real_t const (&invJ)[3][3],
                                       real_t (&gradN)[numNodes][3]);
...
PROXY_HOST_DEVICE
    ...
    // // GEOS_FORCE_INLINE
    // real_t Qk_Hexahedron_Lagrange_GaussLobatto< GL_BASIS >::
...
Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeLocalCoords(
    real_t const (&Xmesh)[8][3], real_t const (&X)[numNodes][3]) {
  int qa, qb, qc;
...
Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeMassTerm(
    int const q, real_t const (&X)[8][3]) {
  int qa, qb, qc;
  GL_BASIS::TensorProduct3D::multiIndex(q, qa, qb, qc);
  const real_t w3D =
...
// GEOS_FORCE_INLINE
PROXY_HOST_DEVICE void
Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeBzMatrix(
    int const qa, int const qb, int const qc, real_t const (&X)[8][3],
    real_t (&J)[3][3], real_t (&B)[6]) {
  jacobianTransformation(qa, qb, qc, X, J);
...
// GEOS_FORCE_INLINE
PROXY_HOST_DEVICE void
Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeBxyMatrix(
    int const qa, int const qb, int const qc, real_t const (&X)[8][3],
    real_t (&J)[3][3], real_t (&B)[6]) {
  jacobianTransformation(qa, qb, qc, X, J);
  real_t const detJ = determinant(J);
...
Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeGradPhiBGradPhi(
    int const qa, int const qb, int const qc, real_t const (&B)[6],
    FUNC &&func) {
  const real_t w =
      GL_BASIS::weight(qa) * GL_BASIS::weight(qb) * GL_BASIS::weight(qc);
...
PROXY_HOST_DEVICE
    // GEOS_FORCE_INLINE
    void
    Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeStiffnessxyTerm(
        int const q, real_t const (&X)[8][3], FUNC &&func) {
  int qa, qb, qc;
  GL_BASIS::TensorProduct3D::multiIndex(q, qa, qb, qc);
  real_t B[6] = {0};
  real_t J[3][3] = {{0}};
...
PROXY_HOST_DEVICE
    // GEOS_FORCE_INLINE
    void
    Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeStiffnesszTerm(
        int const q, real_t const (&X)[8][3], FUNC &&func) {
  int qa, qb, qc;
  GL_BASIS::TensorProduct3D::multiIndex(q, qa, qb, qc);
  real_t B[6] = {0};
...
PROXY_HOST_DEVICE
    ...
    void
    Qk_Hexahedron_Lagrange_GaussLobatto<
        GL_BASIS>::computeFirstOrderStiffnessTerm(int const q,
                                                  real_t const (&X)[8][3],
                                                  FUNC &&func) {
  int qa, qb, qc;
  GL_BASIS::TensorProduct3D::multiIndex(q, qa, qb, qc);
...
PROXY_HOST_DEVICE
    ...
    void
    Qk_Hexahedron_Lagrange_GaussLobatto<
        GL_BASIS>::computeFirstOrderStiffnessTermX(int const q,
                                                   real_t const (&X)[8][3],
                                                   FUNC &&func) {
  int qa, qb, qc;
  GL_BASIS::TensorProduct3D::multiIndex(q, qa, qb, qc);
  real_t J[3][3] = {{0}};
  jacobianTransformation(qa, qb, qc, X, J);
  const real_t detJ = invert3x3(J);
  const real_t w =
      GL_BASIS::weight(qa) * GL_BASIS::weight(qb) * GL_BASIS::weight(qc);

  for (int i1 = 0; i1 < num1dNodes; ++i1) {
    auto val = w * basisGradientAt(i1, qa);
    func(GL_BASIS::TensorProduct3D::linearIndex(i1, qb, qc), q,
...
PROXY_HOST_DEVICE
    ...
    void
    Qk_Hexahedron_Lagrange_GaussLobatto<
        GL_BASIS>::computeFirstOrderStiffnessTermY(int const q,
                                                   real_t const (&X)[8][3],
                                                   FUNC &&func) {
  int qa, qb, qc;
  GL_BASIS::TensorProduct3D::multiIndex(q, qa, qb, qc);
  ...
  for (int i2 = 0; i2 < num1dNodes; ++i2) {
    ...
    func(GL_BASIS::TensorProduct3D::linearIndex(qa, i2, qc), q,
...
// GEOS_FORCE_INLINE
PROXY_HOST_DEVICE void
Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeFirstOrderStiffnessTermZ(
    int const q, real_t const (&X)[8][3], FUNC &&func) {
  int qa, qb, qc;
  GL_BASIS::TensorProduct3D::multiIndex(q, qa, qb, qc);
  real_t J[3][3] = {{0}};
  ...
  for (int i3 = 0; i3 < num1dNodes; ++i3) {
    ...
    func(GL_BASIS::TensorProduct3D::linearIndex(qa, qb, i3), q,
...
// GEOS_FORCE_INLINE
PROXY_HOST_DEVICE void
Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::plusGradNajAij(
    int const q, real_t const (&invJ)[3][3], real_t const (&var)[6],
    real_t (&R)[numNodes][3]) {
  supportLoop(
      q,
      [](real_t const(&dNdXi)[3], int const nodeIndex,
         real_t const(&invJ)[3][3], real_t const(&var)[6],
         real_t(&R)[numNodes][3]) {
        real_t gradN[3] = {0, 0, 0};
        for (int i = 0; i < 3; ++i) {
          for (int j = 0; j < 3; ++j) {
        ...
        R[nodeIndex][0] = R[nodeIndex][0] - var[0] * gradN[0] -
                          var[5] * gradN[1] - var[4] * gradN[2];
        R[nodeIndex][1] = R[nodeIndex][1] - var[5] * gradN[0] -
                          var[1] * gradN[1] - var[3] * gradN[2];
        R[nodeIndex][2] = R[nodeIndex][2] - var[4] * gradN[0] -
                          var[3] * gradN[1] - var[2] * gradN[2];
      },
      invJ, var, R);
}
 ...
 * xi2|
 *                                                                 |===== ===
 * ===         ===| |  0       -1          -1          -1 | |  1   -1/sqrt(5) -1
 * -1 | |  2    1/sqrt(5)      -1          -1 | 60       61         62        63
 * |  3        1          -1          -1 | o---------o---------o---------o |  4
 * -1      -1/sqrt(5)      -1 | 56 /.     57        58        59 /| |  5
 * -1/sqrt(5)  -1/sqrt(5)      -1 | o .       o         o         o | |  6
 * 1/sqrt(5)  -1/sqrt(5)      -1 | 52 /  .   53        54        55 /  | |  7 1
 * -1/sqrt(5)      -1 | o   .     o         o         o   |                  |
 * 8       -1       1/sqrt(5)      -1 | 48 /    o 49      o 50      o 51 /    o
 * |  9   -1/sqrt(5)   1/sqrt(5)      -1 | o---------o---------o---------o     |
 * | 10    1/sqrt(5)   1/sqrt(5)      -1 | |   o .       o         o     |   o |
 * | 11        1       1/sqrt(5)      -1 | |     .                       |     |
 * | 12       -1           1          -1 | | o   o     o   o     o   o   | o   o
 * | 13   -1/sqrt(5)       1          -1 | |     .                       |     |
 * | 14    1/sqrt(5)       1          -1 | o   o .   o   o     o   o     o   o |
 * | 15        1           1          -1 | |     .                       |     |
 * | ..       ..          ..          .. | | o   .     o         o       | o   |
 * | ..       ..          ..          .. | |     o.........o.........o...|.....o
 ...
 * tensor product indices.
 *                                                                  _____________________________________
 *                120      121     122     123       124           |Node xi0 xi1
 * xi2 | o-------o-------o-------o-------o              |=====     === === === |
 *                 /.                              /|              |   0 -1 -1
 * -1 | 115 o .  116o    117o    118o    119o |              |   1   -sqrt(3/7)
 * -1          -1 | /  o                            /  o              |   2 0 -1
 * -1 | 110 o   .111o    112o    113o    114o   |              |   3 sqrt(3/7)
 * -1          -1 | /  o .                          /  o |              |   4 1
 * -1          -1 | 105 o     . o106    o107    o108 109o     |              |
 * 5       -1      -sqrt(3/7)      -1 | /  o   o      102     103    104/  o   o
 * |   6   -sqrt(3/7)  -sqrt(3/7)      -1 | 100
 * o-------o-------o-------o-------o       |              |   7        0
 * -sqrt(3/7)      -1 | | o   o . 101                   | o   o |              |
 * 8    sqrt(3/7)  -sqrt(3/7)      -1 | |       .                       | | | 9
 * 1      -sqrt(3/7)      -1 | o   o   o       o       o       o   o   o |  10
 * -1           0          -1 | |       .                       |       | |  11
 * -sqrt(3/7)       0          -1 | | o   o .20     21      22    23| o   o |24
 * |  12        0           0          -1 | | o.......o.......o.......|.......o
 * |  13    sqrt(3/7)       0          -1 | o   o  ,o       o       o       o o
 * /               |  14        1           0          -1 | |     o       o o o
 * |     o                |  ..       ..          ..          .. | | o  ,15 13
 * 17      18| o  /19               |  ..       ..          ..          .. | |
 * o       o       o       o   |   o                  |  ..       ..          ..
 * .. | o  ,10  o   11  o   12  o   13  o  /14     xi2         | 121        -1
 * 1           1 | | o       o       o       o     | o        |           | 122
 * -sqrt(3/7)      1           1 |
 *          |,5       6       7       8     |/9        | / xi1     | 123 0 1 1 |
 *          o-------o-------o-------o-------o          |/          | 124
 * sqrt(3/7)      1           1 | 0        1       2       3        4 o----- xi0
 * | 125         1          1           1 |
...
/**
 * This class contains the kernel accessible functions specific to the standard
 * Trilinear Hexahedron finite element with a Gauss Lobatto quadrature rule. It
 * is assumed that the indexing for the quadrature points mirrors that of the
 * nodes. Also note that the assumed node ordering is not the standard
 * right-hand-rule used in the literature. Here we use a Cartesian aligned
 * numbering in order to simplify the mapping to the parent coordinates and
 * tensor product indices.
 ...
 * 0       -1                         -1             -1 | o  .     o        o o
 * o        o  |  |  1   -sqrt(1/21(7+/sqrt(7))         -1             -1 | / o
 * /   |  |  2    -sqrt(1/21(7-/sqrt(7))        -1             -1 | 198  / .199
 * 200      201      202      203 /    o  |  3    sqrt(1/21(7-/sqrt(7)) -1 -1 |
 *             o     .  o        o        o        o        o     |  |  4
 * sqrt(1/21(7+/sqrt(7))         -1             -1 | /      . /      |  |  5 1
 * -1             -1 | 192  /   193 o     194      195      196      197 /    o
 * |  |  6       -1                 -sqrt(1/21(7+/sqrt(7)) -1 | o        o o o
 * o        o        o  |  7   -sqrt(1/21(7+/sqrt(7)) -sqrt(1/21(7+/sqrt(7)) -1
 * | /         .                                  /         |  |  8
 * -sqrt(1/21(7-/sqrt(7)) -sqrt(1/21(7+/sqrt(7)) -1 | 186 /    187   .  188 189
 * 190      191 /    o     |  |  9    sqrt(1/21(7-/sqrt(7))
 * -sqrt(1/21(7+/sqrt(7)) -1 | o        o  o     o        o        o        o o
 * |  | 10    sqrt(1/21(7+/sqrt(7)) -sqrt(1/21(7+/sqrt(7)) -1 | /            .
 * /            o  | 11        1                 -sqrt(1/21(7+/sqrt(7)) -1 | 180
 * /    181      . 182    183      184      185 /    o        |  | ..       ..
 * ..             .. | o--------o--------o--------o--------o--------o        o
 * |  | ..       ..                         ..             .. | |           o  .
 * |           o  |  | 204      -1                  sqrt(1/21(7+/sqrt(7)) 1  |
 *    |  o           o        o        o        o  |  o  o        o  | 205
 * -sqrt(1/21(7+/sqrt(7))  sqrt(1/21(7+/sqrt(7)) 1  | |     o        . |     o
 * |  | 206  -sqrt(1/21(7-/sqrt(7))  sqrt(1/21(7-/sqrt(7)) 1  | |        o     .
 * |        o     |  | 207  sqrt(1/21(7+/sqrt(7))   sqrt(1/21(7-/sqrt(7)) 1  |
 *    o           o  .                             o           o  |  | 208
 * sqrt(1/21(7-/sqrt(7))   sqrt(1/21(7+/sqrt(7)) 1  | |  o           . |  o |  |
 * 209       1                  sqrt(1/21(7+/sqrt( *  1  | |     o
 * o--------o--------o--------o--|-----o--------o  | 210      -1 1             1
 * | |        o    ,30       31      32        33 |     34 o    /35 | 211
 * -sqrt(1/21(7+/sqrt(7))          1             1  | o            , o /    |
 * 212  -sqrt(1/21(7-/sqrt(7))          1             1  | |  o        o o o o
 * |  o        o     | 213   sqrt(1/21(7-/sqrt(7))          1             1  |
 *    |     o    ,24       25        26      27    |  28 o    /29    | 214
 * sqrt(1/21(7+/sqrt(7))          1             1  | |         , |         / |
 * 215       1                          1             1  | o        o        o
 * o       o     22 o        o *
 * |_______________________________________________________| |  o    ,18 19 20
 * 21       |  o    /23 |      ,                                     |      / |
 * o        o         o       o        o  |     o o    ,12       13        14 15
 * 16 o    /17 |   ,                                        |   / |  o        o
 * o        o        o     |  o               xi2 | ,6        7        8 9 10 |
 * /11               |
 *    |,                                           |/                  | / xi1
 *    o--------o--------o--------o--------o--------o                   |/
 *    0        1        2        3        4        5                   o-----
