 ...
 */
template <int ORDER, typename TRANSFORM_FLOAT, typename GRADIENT_FLOAT>
...
public:
  ...
  constexpr static TRANSFORM_FLOAT interpolationCoord(const int q,
                                                      const int k) {
    const TRANSFORM_FLOAT alpha =
        (SEMQkGLBasisFunctionsOptim<ORDER, TRANSFORM_FLOAT>::parentSupportCoord(
             q) +
         1.0) /
        2.0;
   ...
   */
  SEMKERNELS_HOST_DEVICE
  constexpr static TRANSFORM_FLOAT
  jacobianCoefficient1D(const int q, const int i, const int k, const int dir) {
    if (i == dir) {
      return k == 0 ? -0.5 : 0.5;
    } else {
   ...
   */
  template <typename COORDS_TYPE, typename JACOBIAN_TYPE>
  static constexpr inline SEMKERNELS_HOST_DEVICE void
  jacobianTransformation(int const qa, int const qb, int const qc,
                         COORDS_TYPE const &X, JACOBIAN_TYPE (&J)[3][3]) {
    for (int k = 0; k < 8; k++) {
      // const int ka = k % 2;
      // const int kb = ( k % 4 ) / 2;
      // const int kc = k / 4;
  ...
  computeGradPhiBGradPhi(int const qa, int const qb, int const qc,
                         TRANSFORM_FLOAT const (&B)[6], FUNC &&func) {
    const TRANSFORM_FLOAT w =
        SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::weight(qa) *
        SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::weight(qb) *
        SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::weight(qc);
    for (int i = 0; i < numSupportPoints1d; i++) {
      ...
      GRADIENT_FLOAT const gia =
          SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::basisGradientAt(
              ORDER, i, qa);
      GRADIENT_FLOAT const gib =
          SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::basisGradientAt(
              ORDER, i, qb);
      GRADIENT_FLOAT const gic =
          SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::basisGradientAt(
              ORDER, i, qc);
      //      printf("i: %d, ibc: %d, aic: %d, abi: %d, gia: %f, gib: %f, gic:
      ...
      for (int j = 0; j < numSupportPoints1d; j++) {
        ...
        GRADIENT_FLOAT const gja =
            SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::basisGradientAt(
                ORDER, j, qa);
        GRADIENT_FLOAT const gjb =
            SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::basisGradientAt(
                ORDER, j, qb);
        GRADIENT_FLOAT const gjc =
            SEMQkGLBasisFunctionsOptim<ORDER, GRADIENT_FLOAT>::basisGradientAt(
                ORDER, j, qc);

        //        printf("j: %d, jbc: %d, ajc: %d, abj: %d, gja: %f, gjb: %f,
  ...
  computeStiffnessAndMassTerm(int const q, COORDS_TYPE const &X, float mass[],
    ...
    TRANSFORM_FLOAT const w3D =
        SEMQkGLBasisFunctionsOptim<ORDER, TRANSFORM_FLOAT>::weight(qa) *
    ...
    //    printf( "B(%d,%d,%d): %18.14e %18.14e %18.14e %18.14e %18.14e
  ...
  computeMassMatrixAndStiffnessVector(const int &elementNumber,
    ...
    for (int q = 0; q < nPointsPerElement; q++) {
      computeStiffnessAndMassTerm(
          q, X, massMatrixLocal,
          [&](const int i, const int j, const GRADIENT_FLOAT val) {
